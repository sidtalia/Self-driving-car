...
void reset(void)              //reset. used almost never after the setup.
{
...
void spiWrite(uint8_t reg, uint8_t data)     //function writes the data in the register of specified address . used once in setup
{
  spiWrite(reg, &data, 1);
}
...
uint8_t spiRead(uint8_t reg)   //function returns the data in the buffer stored at the address passed 
{
  uint8_t buf;
  spiRead(reg, &buf, 1);
  return buf;
